...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updatedContent = await contentCollection.findOne({ _id: new ObjectId(params.id) });

    return NextResponse.json(updatedContent);
  } catch (error) {
    console.error("Error updating content:", error);
    return NextResponse.json({ message: "Failed to update content", error: (error as Error).message }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: "Failed to delete content", error: (error as Error).message }, { status: 500 });
  }
}
