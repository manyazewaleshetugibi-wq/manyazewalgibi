...
export default function WaiterReportPage() {
  ...
  const fetchOrders = useCallback(async (waiterId: string) => {
    try {
      ...
      if (data.success) {
        ...
        const activeOrdersOnly = fetchedOrders.filter((order: Order) => !order.calculated);
        setActiveOrders(activeOrdersOnly);
      } else {
  ...
  const handleCloseTable = async (tableNumber: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const updatedActiveOrders = activeOrders.filter(order => !orderIds.includes(order._id));
  ...
  const applyFilters = useCallback(() => {
    ...
    setCurrentPage(1);
  }, [allOrders, selectedStatus, searchTerm, sortBy, sortOrder]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-50">
      <div className="p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
          ...
          <Button 
            onClick={() => fetchOrders(currentWaitress!._id)} 
            variant="outline" 
            disabled={isLoading}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid gap-3 md:gap-4 grid-cols-2 md:grid-cols-4">
              ...
              <Card className="border-purple-100 shadow-md hover:shadow-lg transition-shadow">
                <CardHeader className="pb-2">
                  <CardTitle className="text-xs md:text-sm font-medium text-purple-900">Average Order</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-sm md:text-2xl font-bold text-purple-900">
                    {formatCurrency(totalOrdersCount ? totalSales / totalOrdersCount : 0)}
                  </div>
                </CardContent>
              </Card>
              <Card className="border-purple-100 shadow-md hover:shadow-lg transition-shadow">
                <CardHeader className="pb-2">
                  <CardTitle className="text-xs md:text-sm font-medium text-purple-900">Total Items</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <Table>
                    <TableHeader className="bg-purple-50">
                      <TableRow>
                        ...
                        <TableHead className="text-purple-900 text-xs md:text-sm">Items</TableHead>
                    ...
                    <TableBody>
                      {paginatedOrders.map((order, index) => (
                        <TableRow key={order._id || `${order.orderNumber}-${index}`} className="hover:bg-purple-50">
                          ...
                          <TableCell className="text-xs md:text-sm">{order.orderItems?.length || 0}</TableCell>
