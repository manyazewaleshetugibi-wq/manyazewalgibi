...
import { LogIn, Phone, Mail, Lock, Fingerprint, Key, User, Shield } from 'lucide-react'
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/customer/verify-otp', {
        ...
        body: JSON.stringify({ email: phoneOrEmail, otp }),
      })
            ...
            <DialogHeader>
              ...
              <motion.div variants={itemVariants}>
                <DialogDescription className="text-center">
                  Enter your credentials for a secure, one-time password login.
                </DialogDescription>
              </motion.div>
            </DialogHeader>
            <Tabs value={loginMethod} onValueChange={(value) => setLoginMethod(value as 'phone' | 'email')} className="w-full mt-6">
              <motion.div variants={itemVariants}>
              ...
              <AnimatePresence mode="wait">
                {!isOtpSent ? (
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    <RainbowButton onClick={handleLogin} className="w-full">
